...
ChatGPT 같은 LLM의 핵심 구조인 **Decoder-only Transformer**를 PyTorch로 처음부터 직접 구현해본 프로젝트입니다. PyTorch에 이미 있는 `nn.MultiheadAttention` 같은 고수준 API를 일부러 사용하지 않고, 어텐션의 핵심인 Query/Key/Value 분할부터 멀티헤드 reshape, causal mask까지 모두 손으로 작성했습니다. "어텐션이 어떻게 동작하는지"를 코드 레벨에서 손에 익히는 것이 1차 목표였고, 구조는 nanoGPT를 참고했습니다.
...
토크나이저 선택도 고민이었습니다. 현대의 GPT는 대부분 BPE 같은 sub-word 토크나이저를 쓰는데, 이번엔 character-level로 갔습니다. "from scratch"의 의미를 살리려면 외부 토크나이저 라이브러리에 의존하지 않는 게 맞다고 봤고, vocab이 65개라 임베딩과 출력 매트릭스가 작아져 학습이 빠르다는 장점도 있었습니다. 단점은 단어 단위 의미 학습이 어렵고, 모델이 단어 경계까지 직접 학습해야 한다는 점입니다. BPE를 직접 구현해보는 것도 다음 프로젝트의 좋은 후보가 될 것 같습니다.

샘플링 전략 비교에서 얻은 가장 큰 인사이트는 "정답이 없다"는 점이었습니다. Greedy는 항상 가장 확률 높은 토큰을 고르니 안정적이지만 단조롭고 반복적입니다. Temperature 1.4로 가면 새로운 단어 조합이 등장하지만 문법이 깨집니다. ChatGPT나 Claude 같은 실제 LLM 서비스의 default temperature가 `0.7~1.0` 사이인 이유를 코드로 직접 검증한 셈입니다. Top-k가 temperature보다 더 안정적인 trade-off를 제공한다는 것도 이번에 처음 체감했습니다.

가장 큰 깨달음은 **GPT의 생성 능력이 결국 단순한 "다음 글자 예측"에서 나온다**는 점입니다. 1.1M characters라는 작은 데이터에서도 모델은 등장인물 형식, 운율, 어휘 분포를 모두 학습합니다. GPT-3가 175B 파라미터에 수백 GB의 텍스트로 학습된다면 그 결과가 어떤 모습일지 이번에 좀 더 직관적으로 가늠할 수 있게 됐습니다. "스케일이 모든 것을 해결한다"는 말이 단순한 슬로건이 아니라는 게 손에 잡히는 느낌입니다.

다음 단계로는 BPE 토크나이저를 직접 구현해보거나, 한국어 코퍼스로 학습해보거나, 절대 위치 임베딩 대신 RoPE(회전 위치 임베딩)를 적용해보는 것이 자연스러운 확장이라고 생각합니다. 이번엔 "기본기"를 다지는 데 집중했으니, 다음엔 응용 단계로 넘어가 보고 싶습니다.
